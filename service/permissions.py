...
class IsClientOwnerOrReadOnly(permissions.BasePermission):
    def has_object_permission(self, request, view, obj):
        if request.method in permissions.SAFE_METHODS:
            return True  # Разрешено для всех при безопасных методах (просмотр)
        return obj.user == request.user or request.user.role == 'admin'  # Редактировать может только автор или администратор
